...
first_feature = ['app_cate_id', 'app_id','inner_slot_id']#inner_slot_id粒度过细
second_feature = ["make", "model", "osv", "adid", "advert_name", "campaign_id", "creative_id",'advert_industry_inner',
                  "carrier", "nnt", "devtype", "os",'city','province']#'campaign_id', 'creative_id'覆盖了基本创意类型因为一个活动id唯一对应是最细的尺度，订单id过细
cross_feature = []
...
adid_nuq = ['model', 'make', 'os', 'city', 'province', 'user_tags', 'app_id', 'carrier', 'nnt', 'devtype',
...
advert_id_nuq = ['model', 'make', 'os', 'city', 'province', 'user_tags', 'app_id', 'carrier', 'nnt','devtype',
                 'app_cate_id', 'inner_slot_id']
for fea in advert_id_nuq:
    gp1 = data.groupby('advert_name')[fea].nunique().reset_index().rename(columns={fea: "advert_name_%s_nuq_num" % fea})
    gp2 = data.groupby(fea)['advert_name'].nunique().reset_index().rename(
        columns={'advert_name': "%s_advert_name_nuq_num" % fea})
    data = pd.merge(data, gp1, how='left', on=['advert_name'])
    data = pd.merge(data, gp2, how='left', on=[fea])
#%%
## app_id 
#439个app_id，而app分类只有23个粒度太粗，一个app_id唯一对应一个app分类，与其他特征无次关系，值得挖掘其种类数量特征
app_name_nuq = ['model', 'make', 'os', 'city', 'province', 'user_tags', 'carrier', 'nnt', 'devtype',
              'app_cate_id', 'inner_slot_id']
for fea in app_name_nuq:
...
for feat_1 in ['advert_id', 'advert_industry_inner', 'advert_name', 'campaign_id', 'creative_height',
               'creative_tp_dnf', 'creative_width', 'province', 'app_id']:
    res = pd.DataFrame()
...
res = pd.read_table(f'{path}/round1_iflyad_test_feature.txt').loc[:, ['instance_id']]
